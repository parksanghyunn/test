...
//	alert("호출!!");
	...
	for (var i = 0; i < frm.hobby.length; i++) {
		if(frm.hobby[i].checked){
			//만약 한개라도 체크되어 있으면 들어온다
			break;
		}
	}
	//위에 for문이 break를 만나서 탈출했다면,
	//i는 frm.hobby.length까지 증가하지 못하고 탈출하게 된다
	//만약 정상 탈출이라면 i == frm.hobby.length 이고 하나도 체크되지 않았다는 의미이다
	if(i == frm.hobby.length ){
		alert("취미는 1개 이상 입력해 주세요");
		return false;
	}
...
function checkId(id) {
	...
	var idcheck = document.getElementById("checkId_text");
	...
	httpRequest.onreadystatechange = function() {
		if(httpRequest.readyState == XMLHttpRequest.DONE && httpRequest.status == 200){
			if(httpRequest.responseText.trim() == "ok"){
				idcheck.innerHTML = "사용할수 있는 아이디입니다";
			}else{
				idcheck.innerHTML = "이미 있는 아이디 입니다";
			}
		}
	}
