...
<body class="skin-purple" style="margin-top: 80px; overflow-x:hidden">
			...
			<div class="row">
				<ul class="nav nav-tabs nav-justified" role="tablist">
					<li class="active"><a href="#board_1" aria-controls="board_1"
						role="tab" data-toggle="tab">게시글 신고 접수</a></li>
				</ul>
							...
							<c:when test="${list != null and fn:length(list) > 0 }">
								<c:forEach var="ad_bean1" items="${list}" >
									...
									<tr>
										...
										<td>
											<input name="blind" type="button" value="블라인드" onclick="cutBoard(${ad_bean1.getReport_uid()},${ad_bean1.getBoard_idx()},${ad_bean1.getBoard_state()},2,1)" id="blindBtn${ad_bean.getReport_uid()}" style="border:none; outline:none">
											<input name="cancle" type="button" value="취소" onclick="cutBoard(${ad_bean1.getReport_uid()},${ad_bean1.getBoard_idx()},${ad_bean1.getBoard_state()},0,1)" style="border:none; outline:none">
										</td>
									</tr>
							...
							<ul class="pagination" style="margin-left:63px;">
								<c:choose>
                  					<c:when test="${nowPage > 1}">
                   					  <a href="/AdminBoardReportReceiveOk.ad?page=${nowPage - 1}&report_state=1">[이전]</a>&nbsp;
                 					 </c:when>
            				   </c:choose>
								<c:forEach var="i" begin="${startPage}" end="${endPage}">
                    				 ...
                    				 <c:otherwise>
                   			  			   <a href="/AdminBoardReportReceiveOk.ad?page=${i}&report_state=1">${i}&nbsp;</a>
                   					  </c:otherwise>
                  					</c:choose>
            			 	 	 </c:forEach>
								<c:choose>
                 				 <c:when test="${nowPage != totalPage}">
                  				   <a href="/AdminBoardReportReceiveOk.ad?page=${nowPage + 1}&report_state=1">[다음]</a>&nbsp;
                				  </c:when>
              					 </c:choose>
							</ul>
						</div>
	...
	<script>
		...
		function cutBoard(rUid, board_idx, board_state , state,pageState){
			...
			if(state == 2){
				var checkConfirm = confirm("게시판을 블라인드 처리하시겠습니까?");
			}else{
