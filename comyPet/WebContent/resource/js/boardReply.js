...
$("#write_btn").on("click", function() {
	...
	var sendIdx = $("#reply_data_idx").val();
	var sendState = $("#reply_data_state").val();
	var conuntdown = setInterval(function(){
		$("#counts").html(count);
		if(count == 0){
			clearInterval(conuntdown);
			$.ajaxSettings.traditional = true;
			$.ajax({
				url : "/boardReplyWriteOkAction.bo",
				type : "POST",
				traditional : true,
				data : { title : sendTitle, contents : sendContents, state : sendState, idx : sendIdx, insertPath : insertPath, uploadPath : uploadPath},
				dataType : "json",
				success : function(data){
					if(data["code"] == "200"){
						alert("게시글 수정이 완료되었습니다.");
						location.href = "/boardListOk.bo?board_state="+sendState;
					}else{
						alert("게시글 등록 실패");
						location.reload();
					}
