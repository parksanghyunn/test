...
//전송방식에 상관없이, 실행되는 로직은 동일하므로 doProcess()를 구현하여 각 메소드에 사용해준다.
public class MemberFrontController extends HttpServlet{

	/**
	 * JVM의 메모리에서만 머물러 있던 객체를 시스템이 종료되거나 네트워크로 통신을 할 때에도 그대로 사용할 수 있도록
	 * 영구화 목적으로 직렬화를 사용한다.
	 * 직렬화된 객체는 byte형태로 변환되어 있으며, 다시 사용하고 싶을 때에는 역직렬화를 통해서 객체로 변환된다.
	...
	protected void doProcess(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		case "/member/MemberJoinOk.me": //회원가입 완료 버튼 클릭 시
			...
			break;
		case "/member/MemberCheckIdOk.me" : //회원가입 시 아이디 중복체크
			try {
